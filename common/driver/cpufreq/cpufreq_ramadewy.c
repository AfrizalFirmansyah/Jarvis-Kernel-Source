/*
 * drivers/cpufreq/cpufreq_ramadewy.c
 *
 ...
 *
 * Author: Afrizal Firmansyah (firmansyahafrizal@gmail.com)
 *
 */
...
static int cpufreq_governor_ramadewy(struct cpufreq_policy *policy,
		unsigned int event);

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_RAMADEWY
static
#endif
struct cpufreq_governor cpufreq_gov_ramadewy = {
	.name = "interactiveX",
	.governor = cpufreq_governor_ramadewy,
...
static void cpufreq_ramadewy_timer(unsigned long data)
{
	u64 delta_idle;
	u64 update_time;
 ...
 */
static unsigned int cpufreq_ramadewy_calc_freq(unsigned int cpu)
{
...
static void cpufreq_ramadewy_freq_change_time_work(struct work_struct *work)
{
	unsigned int cpu;
...
//			__cpufreq_driver_target(policy, target_freq, CPUFREQ_RELATION_H);
			...
			target_freq = cpufreq_ramadewy_calc_freq(cpu);
...
static struct attribute *ramadewy_attributes[] = {
	&min_sample_time_attr.attr,
	NULL,
};

static struct attribute_group ramadewy_attr_group = {
	.attrs = ramadewy_attributes,
	.name = "RamaDewy",
};

static void ramadewy_suspend(int suspend)
{
	...
	if (!enabled) return;
		...
		suspended = 0;
                ...
                pr_info("[imoseyon] RamaDewy awake at %d\n", policy->cur);
        } else {
		suspended = 1;
                __cpufreq_driver_target(policy, policy->min, CPUFREQ_RELATION_L);
                pr_info("[imoseyon] RamaDewy suspended at %d\n", policy->cur);
        }
}

static void ramadewy_early_suspend(struct early_suspend *handler) {
     ramadewy_suspend(1);
}

static void ramadewy_late_resume(struct early_suspend *handler) {
     ramadewy_suspend(0);
}

static struct early_suspend ramadewy_power_suspend = {
        .suspend = ramadewy_early_suspend,
        .resume = ramadewy_late_resume,
        .level = EARLY_SUSPEND_LEVEL_DISABLE_FB + 1,
};

static int cpufreq_governor_ramadewy(struct cpufreq_policy *new_policy,
		unsigned int event)
{
	int rc;
	...
	case CPUFREQ_GOV_START:
		...
		policy = new_policy;
		enabled = 1;
        	register_early_suspend(&ramadewy_power_suspend);
	...
	case CPUFREQ_GOV_STOP:
		...
		enabled = 0;
        	unregister_early_suspend(&ramadewy_power_suspend);
        	pr_info("[imoseyon] RamaDewy inactive\n");
			break;
...
static int __init cpufreq_ramadewy_init(void)
{
	unsigned int i;
	struct timer_list *t;
	...
	for_each_possible_cpu(i) {
		...
		t->function = cpufreq_ramadewy_timer;
		t->data = i;
	}

	/* Scale up is high priority */
	up_wq = create_workqueue("kramadewy_up");
	down_wq = create_workqueue("knramadewy_down");

	INIT_WORK(&freq_scale_work, cpufreq_interactivex_freq_change_time_work);

        pr_info("[imoseyon] RamaDewy enter\n");
	return cpufreq_register_governor(&cpufreq_gov_ramadewy);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_RAMADEWY
fs_initcall(cpufreq_ramadewy_init);
#else
module_init(cpufreq_ramadewy_init);
#endif

static void __exit cpufreq_ramadewy_exit(void)
{
        pr_info("[imoseyon] RamaDewy exit\n");
	cpufreq_unregister_governor(&cpufreq_gov_ramadewy);
	destroy_workqueue(up_wq);
	destroy_workqueue(down_wq);
}

module_exit(cpufreq_ramadewy_exit);

MODULE_AUTHOR("Afrizal Firmansyah <firmansyahafrizal@gmail.com>");
MODULE_DESCRIPTION("'cpufreq_RamaDewy' - A cpufreq governor for "
